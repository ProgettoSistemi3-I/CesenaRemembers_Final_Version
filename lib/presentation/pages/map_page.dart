...
class _MapPageState extends State<MapPage> with WidgetsBindingObserver {
  ...
  final String _urlStandard = 'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png';
  final String _urlSatellite = 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}';
  ...
  Marker _buildMarkerFromPoi(Poi poi) {
    Color iconColor;
    switch (poi.type) {
      case 'school': iconColor = Colors.red; break;
      case 'bridge': iconColor = Colors.green; break;
      case 'library': iconColor = Colors.orange; break;
      default: iconColor = Colors.blue;
    }
    ...
    _serviceStatusStreamSubscription = Geolocator.getServiceStatusStream().listen((ServiceStatus status) {
      ...
      if (status == ServiceStatus.disabled) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("GPS disattivato"), backgroundColor: Colors.red, duration: Duration(seconds: 3)),
        );
      }
    });
  }
        ...
        children: [
          FlutterMap(
            ...
            options: MapOptions(
              ...
              interactionOptions: InteractionOptions(flags: _isMapLocked ? InteractiveFlag.none : InteractiveFlag.all),
            ...
            children: [
              ...
              CurrentLocationLayer(
                ...
                style: LocationMarkerStyle(
                  marker: const DefaultLocationMarker(color: Colors.blue, child: Icon(Icons.navigation, color: Colors.white, size: 14)),
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      if (_isMapMenuOpen) ...[
                        Row(
                          children: [
                            _buildMapTypeSquare(title: 'Standard', icon: Icons.map, bgColor: Colors.grey.shade100, url: _urlStandard),
                            const SizedBox(width: 12),
                            _buildMapTypeSquare(title: 'Satellite', icon: Icons.satellite_alt, bgColor: Colors.green.shade100, url: _urlSatellite),
                          ],
                        ),
                        const SizedBox(height: 12),
                      ],
                      FloatingActionButton.extended(
                        heroTag: "btnMapType", backgroundColor: Colors.white, elevation: 4,
                        icon: Icon(_isMapMenuOpen ? Icons.close : Icons.layers, color: Colors.black87),
                        label: Text(_isMapMenuOpen ? "Chiudi" : "Mappe", style: const TextStyle(color: Colors.black87)),
                        onPressed: () => setState(() => _isMapMenuOpen = !_isMapMenuOpen),
                      ),
                    ],
                  ),
                ),
                Positioned(
                  ...
                  child: FloatingActionButton(
                    heroTag: "btnLockMap", backgroundColor: Colors.white, elevation: 4, shape: const CircleBorder(),
                    child: Icon(_isMapLocked ? Icons.lock : Icons.lock_open, color: _isMapLocked ? Colors.red : Colors.black54),
                    onPressed: () => setState(() => _isMapLocked = !_isMapLocked),
                  ),
                ),
                Positioned(
                  right: 20, bottom: 20,
                  child: FloatingActionButton(
                    heroTag: "btnMyLocation", backgroundColor: Colors.white, elevation: 4, shape: const CircleBorder(),
                    child: Icon(Icons.my_location, color: _alignPositionOnUpdate == AlignOnUpdate.always ? Colors.blue : Colors.black54),
                    onPressed: () {
